...
if (!function_exists('number_down')) {
    /**
     * 保留指定位小数并向下取整
     * @param $number
     * @param int $scale
     * @param bool $removeInvailZero
     * @return float
     */
    function number_down($number, $scale = 2, $removeInvailZero = true)
    {
        if ($scale <= 0) {
            return floor($number);
        }
        $multiplier = pow(10, $scale);
        $new_number = bcmul($number, $multiplier, $scale + 2);
        $new_number = bcdiv(floor($new_number), $multiplier, $scale);
        $new_number = number_format($new_number, $scale, '.', '');
        return $removeInvailZero ? floatval(rtrim($new_number, '0')) : $new_number;
    }
}

if (!function_exists('add_script_label')) {
    /**
     * 为JS代码添加 script 标签
     * @param $js_code
     * @return string
     */
    function add_script_label($js_code)
    {
        if (substr($js_code, 0, 8) != '<script>') {
...
if (!function_exists('create_order_sn')) {
    ...
    {
        $redis_key = \app\common\library\Redis::REDIS_KEY_ORDER_SN_SEQUENCE_NUMBER . date('Ymd');
        $sequence_number = \app\common\library\Redis::getInstance()->incr($redis_key);
        if ($sequence_number == 1) {
            \app\common\library\Redis::getInstance()->expire($redis_key, \app\common\library\Redis::REDIS_KEY_EXPIRE_TWO_DAYS);
        }
        return date('YmdHi') . str_pad($sequence_number, 3, '0', STR_PAD_LEFT);
    }
}
...
if (!function_exists('format_duration')) {
     ...
     * 格式化时长
     * @param $duration
     * @param bool $default_show_hours
     * @return string
     */
    function format_duration($duration, $default_show_hours = false)
    {
        $hours = floor($duration / 3600);
        $minutes = floor(($duration % 3600) / 60);
        $seconds = $duration % 60;
        if ($hours > 0 || $default_show_hours) {
