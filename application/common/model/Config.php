...
class Config extends Model
{
    ...
    public static function updateValueByName($name, $value)
    {
        ...
        if (self::where('id', $item->id)->setField(['value' => $value])) {
            self::refreshFile(false);
        }
        return true;
    }
     ...
     * 刷新配置文件
     */
    public static function refreshFile($verify_auth = true)
    {
        //如果没有配置权限无法进行修改
        if ($verify_auth && (!\app\admin\library\Auth::instance()->check('general/config/edit'))) {
            return false;
        }
        $config = [];
        $configList = self::all();
